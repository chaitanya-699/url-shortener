...
/**
 * Masks the last 3 digits of an IP address for security and privacy
 * This prevents full IP address exposure while still showing general location info
 * @param {string} ip - The IP address to mask
 * @returns {string} - Masked IP address (e.g., "192.168.1.xxx", "2001:db8:85a3::xxx")
 */
export const maskIpAddress = (ip) => {
  ...
  // Handle IPv6 addresses (replace last segment)
  if (ip.includes(":")) {
    const parts = ip.split(":");
    if (parts.length > 1) {
  ...
  // If it's not a recognizable IP format, mask the last 3 characters
  if (ip.length > 3) {
    ...
    try {
      ...
      if (fetchError.name === "AbortError") {
        return {
          isAccessible: false,
          error:
            "URL request timed out. The website may be slow or unreachable.",
        };
      }
      ...
      try {
        ...
        if (
          ...
          return {
            isAccessible: false,
            error:
              "Unable to reach the website. Please check if the URL is correct.",
          };
        }
    ...
    if (!accessibilityResult.isAccessible) {
      ...
      if (domainExists) {
        return {
          ...
          isAccessible: true,
          warning:
            "URL appears to be valid, but accessibility could not be fully verified. Proceeding anyway.",
        };
      }
      ...
      return {
        ...
        error:
          ...
          "The website appears to be unreachable or does not exist.",
      };
    }
